describe("Smoother Service", function() {
	...
	describe("Standard Filter", function() {
		var spikeSpy, smoothSpy, points;
		beforeEach(function() {
			spikeSpy = spyOn(service, 'removeSpikes').and.callThrough();
			smoothSpy = spyOn(service, 'smoothLine').and.callThrough();
			points = service.standardFilter(spikePoints);
		});
		it("should run the correct filters", function() {
			expect(spikeSpy.calls.argsFor(0)).toEqual([spikePoints,160,5]);
      expect(smoothSpy.calls.argsFor(0)[1]).toEqual(20)
      expect(spikeSpy.calls.argsFor(1).slice(1,3)).toEqual([140,3])
		})
    it("should return at least 3 points", function(){
